...
use crate::protocols::tcp::{TcpInfo};
...
/*
impl Verify for IpInfo {
    type Rule: Ip
    fn check_rule(&self, rule: &Ip) -> bool {
        match rule {
            Ip::Origin(regex) => match self.ip_origin {
                Some(ip) => true,
                None => false,
            }
            Ip::Destination(regex) => match self.ip_destination {
                Some(ip) => true,
                None => false,
            }
            Ip::L4(l4) => match self.l4 {
                L4Info::Tcp(_) => *l4 == L4::Tcp,
                L4Info::Udp(_) => *l4 == L4::Udp,
            }
        }
